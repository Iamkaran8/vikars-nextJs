...
//     useEffect(() => {
//         if (id) {
//             dispatch(FetchSingleBlog({ id: id as string }));
//         }
//     }, [dispatch, id]);
...
//                     <p className="text-gray-500">{new Date(singleBlog.date).toDateString()}</p>
//                     <img
//                         src={singleBlog.image}
//                         alt={singleBlog.title}
//                         className="w-full max-h-[500px] object-cover rounded-2xl my-6"
//                     />
//                     <p className="text-18 font-medium text-[#353434]">{singleBlog.description}</p>
//                 </div>
//             </div>
//         </div>
//     );
...
export const SinglePost = () => {
  const { id: slugWithId } = useParams(); // slugWithId = "the-future-of-ai-673a7b12c3"
  const dispatch = useAppDispatch();
  ...
  useEffect(() => {
    if (slugWithId) {
      // Extract actual MongoDB _id (part after the last '-')
